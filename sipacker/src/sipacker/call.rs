...
use states::{Incoming, Outgoing, Established};
    ...
    mod states {
        ...
        impl WaitingForAction {
            ...
            pub async fn send_accept(
                ...
                self.send_action(IncomingCallAction::Accept { audio_sender, audio_receiver }).await
            }
            
            pub async fn send_decline(self, reason: &str) -> Result<Call> {
                self.send_action(IncomingCallAction::Decline { reason: reason.to_owned() }).await
            }
            ...
            async fn send_action(self, action: IncomingCallAction) -> Result<Call> {
                ...
                let incoming = Incoming::from(
                    WaitingForActionResponse::new(self.calling_task)
                );
                Ok(incoming.into())
            }
            ...
            async fn run_calling_task(
                ...
                let action = select! {
                    ...
                    _ = cancellation.cancelled() => Some(
                        IncomingCallAction::Decline{ reason: "The call cancelled".to_owned()}
                    ),
                };
